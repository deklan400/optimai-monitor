(function(){
  ...
  function stackedChart(rows){
    ...
    for(let i=0;i<=4;i++){
      ...
      grid+=`<line class="activity-grid" x1="${L}" y1="${yy}" x2="${W-R}" y2="${yy}"/>`;
      ticks+=`<text class="activity-tick" x="${L-10}" y="${yy+4}" text-anchor="end">${fmt(val)}</text>`;
    }
    const bars=rows.map((x,i)=>{
      const cx=L+step*i+step/2,bx=cx-bar/2;
      let top=T+PH;
      const parts=[['pending','activity-pending'],['retried','activity-retry'],['submitted','activity-submit'],['failed','activity-failed']];
      let out='';
      for(const [key,cls] of parts){
        const hh=h(x[key]);
        top-=hh;
        if(hh>0)out+=`<rect class="${cls}" x="${bx}" y="${top}" width="${bar}" height="${hh}" rx="4"><title>${x.date} ${key}: ${fmt(x[key])}</title></rect>`;
      }
      const total=n(x.submitted)+n(x.pending)+n(x.retried)+n(x.failed);
      const d=sd(x.date).split('\n');
      return `${out}<text class="activity-value" x="${cx}" y="${Math.max(16,top-8)}" text-anchor="middle">${fmt(total)}</text><text class="activity-tick" x="${cx}" y="${H-40}" text-anchor="middle">${d[0]}</text><text class="activity-tick" x="${cx}" y="${H-18}" text-anchor="middle">${d[1]||''}</text>`;
    }).join('');
  ...
  async function activityRenderCharts(){
    if(typeof currentPage!=='undefined'&&currentPage!=='charts')return;
  ...
  window.renderCharts=activityRenderCharts;
  try{renderCharts=activityRenderCharts}catch(e){}
  setTimeout(function(){
    try{
      const chartPage=document.getElementById('charts');
      if(chartPage&&!chartPage.classList.contains('hidden'))activityRenderCharts();
    }catch(e){}
  },700);
})();
